...
1. 读取已分析完成的 iOS JSON specs 和平台适配产物（包括 platform-adaptation.json.interaction_adaptation）。
...
6. 按 `screens.json` 的 `layout_spec` 和 `component_specs` 实现 ArkUI 页面。`layout_spec` 是 UI 生成的主要输入；`runtime_ui_tree_evidence` 用于核对运行时真实可见元素和页面状态；`screenshot_evidence` / `visual_notes` 用于补充视觉细节。不允许只实现信息不完整的框架 UI。
...
| 核心功能 | 由 harmony-implementation.json.module_plan 决定 | 根据 features.json 和 modules.json 中的实际模块划分 |
...
`functions.json` 中每个标注了 `concurrency`（非 `none`）的函数，其 Harmony 实现必须使用对应的并发模式。映射关系查 `platform-capabilities.json` 中 `category: "concurrency"` 的条目。
...
11. `functions.json` 中标注了 `concurrency` 的函数，其 Harmony 实现必须使用对应的并发模式，不允许退化为同步或串行。
...
**数据来源**：
- 颜色 hex 值 ← `ios-spec.json.resources` 中 `type: "color"` 的条目（含 `value`、`value_light`、`value_dark`）
- 字号 ← `ios-spec.json.screens` 的 `layout_spec` 中各 element 的 `font_size` 字段
- SF Symbol ← `ios-spec.json.resources` 中 `type: "sf_symbol"` 的条目

**注册流程**：

1. 读取 `ios-spec.json.resources`，提取所有颜色条目。
...
4. 写入 `entry/src/main/resources/dark/element/color.json`（暗色值）。如果 `resources.json` 提供了 `value_dark`，直接使用；否则根据亮色值推算暗色配对（浅色→深色反转，强调色保持不变）。
...
3. **iOS 源码中的颜色值必须先注册为资源再使用**。从 `screens.json` 的 `layout_spec`、`resources.json` 提取所有颜色 hex 值，注册到 `color.json`（亮色）和 `dark/element/color.json`（暗色），然后在代码中用 `$r()` 引用。
...
如果 `screens.json` 中存在 `runtime_ui_tree_evidence`，生成页面时必须读取并用于校准：
...
- 页面实现必须以 `screens.json` 中的 `screenshot_evidence` / `visual_notes` 为视觉基准，争取高度还原。
...
所有阶段都必须读取 `platform-adaptation.json.interaction_adaptation`，并在自己的模块范围内实现对应的导航和交互要求。`functions.json` 中标注了 `concurrency` 的函数必须查 `platform-capabilities.json` 的 concurrency 条目实现对应并发模式。
