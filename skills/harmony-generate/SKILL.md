...
- `output_{{PROJECT_NAME}}/ios-analyze/reports/ios工程分析报告.md`
...
`ios工程分析报告.md` 是辅助输入，用于快速理解应用主流程、UI 还原重点、数据链路和风险。代码生成必须以 `ios-spec.json` 和 `platform-adaptation.json` 的稳定 id 为准；如果报告提到的功能、页面、能力或资源在 spec 中没有对应 id，不能自由实现，必须记入 `harmony-implementation.json.external_requirements` 或追踪表缺口。
...
实现顺序和完整性检查：
1. 读取 `ios-spec.json.features`，列出全部 feature（一级功能→二级功能→三级功能点）。
