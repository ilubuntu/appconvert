...
4. 接入真实数据源，新闻流真实数据优先，固定样例数据 只做网络失败、解析失败或截图稳定性兜底。
5. 按 `output/ios-analyze/ios界面清单.md` 和截图实现 ArkUI 页面，不允许只实现信息不完整的框架 UI。
6. 按 `output/ios-map/ios-harmony-kit映射.md` 接入平台能力或工程侧适配层。
7. 生成集成追踪表，明确每个 iOS 类型/函数的 Harmony 去向。
8. 使用本地 HarmonyOS 工具链构建，并用精确文件和命令报告失败。

## 实现规则

- 每个页面必须能追溯到 `output/ios-analyze/ios功能清单.md`。
- 每个 Harmony 模块必须能追溯到 `output/ios-analyze/ios模块结构.md`。
- 每个 Harmony 文件必须能追溯到 `output/ios-analyze/ios源码索引.md`。
- 每个核心函数必须能追溯到 `output/ios-analyze/ios函数级清单.md`。
- 每个系统能力必须能追溯到 `output/ios-map/ios-harmony-kit映射.md`。
- 每个 UI 决策必须能追溯到 `output/ios-analyze/ios界面清单.md` 或截图产物。
- 真实数据接入属于本 skill 的职责，不能留给单独后置阶段。
- 不允许只写 mock、固定样例数据 或 `example.com` 假数据来冒充迁移完成。
- RSS/Atom 网络请求和 XML 解析必须有可运行链路；固定样例数据 只能兜底。
...
| 子阶段 | 主要输出 | 禁止事项 |
| --- | --- | --- |
| 模块计划 | `output/harmony-generate/harmony模块实现计划.md` | 不写大段业务代码 |
