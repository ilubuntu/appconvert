...
1. **数据源必须与 iOS 一致**。iOS 用哪些 RSS URL、API endpoint、查询参数，HarmonyOS 就用哪些，从 `functions.json` 的 `data_sources` 和 iOS 源码中提取。
...
4. **数据获取失败的排查优先级高于 UI 还原**。首页能显示真实新闻 > 首页样式完美但没有数据。
...
if (this.store.errorMessage.length > 0) {
  Column({ space: 16 }) {
    ...
    Button('Retry').onClick(() => { this.store.fetchAllNews() })
  }
...
8. **不允许跳过中间页面**。如果 navigates_to 的 target 是 `screen.article.detail`，文章卡片点击必须跳转到 ArticleDetailPage，不能直接跳到 ArticleWebPage。ArticleDetailPage 内部的 "Read Full Article" 按钮才跳转到 ArticleWebPage。导航层级：文章卡片 → ArticleDetailPage → (Read Full Article 按钮) → ArticleWebPage。
...
5. **条件状态**：layout_spec 中标记了 `condition` 的 section，其 else 分支也必须有 UI（如 ForYou 的"Building Your Feed"空态）。
...
1. **WatchLaterStore 的 items 必须持久化**。toggle/remove/markAsRead 后调用 `PersistenceStore.save()`。
2. **KeywordAlertStore 的 alerts 必须持久化**。add/remove/toggle 后保存。
3. **CustomFeedStore 的 feeds 必须持久化**。add/remove 后保存。
4. **SettingsManager 的 settings 必须持久化**。每次修改后保存。
...
3. **如果 iOS 源码中一个设置值在某个组件中被读取**（如 ArticleCard 读 `showSentimentColors` 决定是否显示徽章），Harmony 侧对应组件必须读取同样值产生同样行为。
...
5. **navigation_bar 必须实现**：`navigation_bar.title` → `Navigation` 的 `.title()`，`trailing_items` → `.menus()` 或自定义 toolbar。**根 Tab 页面的 Navigation 必须加 `.hideBackButton(true)`**（Tab 首页没有上级页面，不需要返回按钮）。
6. **tab_bar 必须实现且必须带图标**：根页面的 `tab_bar` → `Tabs()` 组件，每个 tab item 必须有 icon + label。`tab_bar.items[].icon` 来自 layout_spec，是 iOS SF Symbol 名，必须按下表映射为 HarmonyOS 图标或 emoji。不允许只写文字 tab。
   ...
   **tabBar 正确写法（图标和文字分两行，不是拼成一个字符串）**：
   ...
   // 使用：
   TabContent() { ... }.tabBar(this.TabBuilder('\u{1F4F0}', 'Home'))

   // ❌ 错误：图标和文字拼成一个字符串，会显示 ...
   TabContent() { ... }.tabBar('\u{1F4F0} Home')
   ```
...
3. **数据为空时必须显示错误状态**：如果 fetchAllNews 结果为空数组，页面必须显示错误提示文本和重试按钮，不是空白页面。
...
- RSS/Atom 网络请求和 XML 解析必须有可运行链路，数据必须来自真实 URL。
