...
- ArkTS 数据模型、服务和页面
- 真实数据加载链路
- 参照 `screens.json` 中视觉证据实现的高保真 ArkUI 页面
- 构建/测试结果摘要
- 全量实现追踪表和外部配套要求
...
1. 读取已分析完成的 iOS JSON specs 和平台适配产物（包括 interaction-adaptation.json）。
...
**数据来源**：
- 颜色 hex 值 ← `specs/resources.json` 中 `type: "color"` 的条目（含 `value`、`value_light`、`value_dark`）
- 字号 ← `specs/screens.json` 的 `layout_spec` 中各 element 的 `font_size` 字段
- SF Symbol ← `specs/resources.json` 中 `type: "sf_symbol"` 的条目

**注册流程**：

1. 读取 `specs/resources.json`，提取所有颜色条目。
...
- 每个 UI 决策必须能追溯到 `specs/screens.json` 的 `layout_spec`、`runtime_ui_tree_evidence`、`screenshot_evidence` 或 `visual_notes`。
...
所有阶段都必须读取 `interaction-adaptation.json`，并在自己的模块范围内实现对应的导航和交互要求。`functions.json` 中标注了 `concurrency` 的函数必须查 `platform-capabilities.json` 的 concurrency 条目实现对应并发模式。
