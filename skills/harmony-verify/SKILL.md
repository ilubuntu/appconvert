...
`ios-report.md` 是辅助输入，用于理解主流程、UI 还原重点和风险背景。验证判定必须以 `ios-spec.json`、`platform-adaptation.json` 和 `harmony-implementation.json` 的结构化字段为准；报告中有但 spec 中没有稳定 id 的内容，只能记录为分析规格缺口，不能直接作为验收项。
...
- 不重新分析 iOS 工程。
- 不重新生成 Harmony 代码。
- 不修改工程文件（只读验证）。如果发现缺口，记入 `verification.json.gaps[]`，由 harmony-generate 修复后重新验收。
...
读取 `ios-spec.json.features`，逐个 feature 检查：

- 是否在 Harmony 工程中找到对应实现文件。
- 对应文件中是否有实质代码（不是空函数或 TODO）。
- `acceptance[].description` 列出的验收标准是否可从代码中推断为已实现。

每个 feature 的验证结果：`passed` / `missing` / `partial`。

### 3. 导航可达验证

读取 `ios-spec.json.screens` 的 `navigates_to` 和 `platform-adaptation.json.interaction_adaptation` 的 `navigation_map`，逐条检查：
...
读取 `ios-spec.json.screens` 的 `interactions` 和 `platform-adaptation.json.interaction_adaptation` 的 `interactions`，逐条检查：
...
读取 `ios-spec.json.screens[].layout_spec.sections[]`，逐个检查：

- 每个 section 是否有对应 ArkUI 结构代码。
- `sections[].data_binding` 是否连接到 Store / State，而不是静态假数据。
- `sections[].row.elements[]` 是否在列表行 UI 中体现。
- `sections[].row.actions[]` 是否生成对应 tap、swipe、context menu 等交互。
- `empty_state_ref` 指向的空态是否实现并在空数据时可显示。
- 页面中的 `resource_ref` 是否能对应到 Harmony resources 或 `symbol_map`。

每个 section 的验证结果：`passed` / `missing_section` / `static_data` / `row_incomplete` / `action_missing` / `resource_missing`。

### 5. 并发还原验证

读取 `ios-spec.json.functions` 中所有 `concurrency` 不为 `none` 的函数，逐个检查：
...
读取 `ios-spec.json.features` 中所有 feature 的 `data_sources`，筛选 `type` 为 `network` 的条目，检查：
...
读取 `platform-adaptation.json.implementation_guidance` 的 `platform_modules`，逐个检查：
...
读取 `platform-adaptation.json.feature_adaptation`，检查：

- 每个 `adaptation_items[]` 条目的 `generation_tasks[]` 是否有对应的代码实现。
- `strategy: "unsupported_pending_decision"` 的条目是否有明确的处理决策（不是静默跳过）。
- `risk_level: "high"` 的条目：`fallback` 必须有对应的代码实现或 adapter 层。
- `risk_level: "medium"` 的条目：`fallback` 至少有工程侧入口或 TODO 标记。

每个能力/高风险的验证结果：`passed` / `mapped_but_missing` / `mitigated` / `not_mitigated` / `partially_mitigated`。

### 8. layout_spec 还原验证

读取 `ios-spec.json.screens` 的 `layout_spec`，逐个页面检查：
...
- **每个 screen 必须有对应的页面文件**。ios-spec.json.screens 中每个 screen id 的 ios_view 必须在 Harmony 工程中有对应的 .ets 页面文件。
...
读取 `ios-spec.json.screens` 的 `runtime_ui_tree_evidence`，逐个页面检查：
...
读取 `ios-spec.json.screens` 的 `layout_spec` 中每个 element 的 `effect` 字段，检查：
...
Step 4. 逐维度验证，记录每项结果
Step 5. 输出 verification.json（包含 gaps[]）
Step 6. 按需输出人工摘要
```
...
```json
{
  ...
  "summary": {
    ...
    "total_adaptation_items": 8,
    "passed_adaptation_items": 7,
    "total_high_risk_items": 5,
    "mitigated_high_risk_items": 4,
  ...
  "features": [
    {
      ...
      "harmony_files": [],
      "acceptance_results": [
        { "criterion_id": "", "description": "", "status": "passed" }
      ]
    }
  ],
  "navigations": [
