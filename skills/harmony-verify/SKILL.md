...
`ios工程分析报告.md` 是辅助输入，用于理解主流程、UI 还原重点和风险背景。验证判定必须以 `ios-spec.json`、`platform-adaptation.json` 和 `harmony-implementation.json` 的结构化字段为准；报告中有但 spec 中没有稳定 id 的内容，只能记录为分析规格缺口，不能直接作为验收项。
