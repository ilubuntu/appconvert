...
读取 `screens.json` 的 `navigates_to` 和 `platform-adaptation.json.interaction_adaptation` 的 `navigation_map`，逐条检查：
...
读取 `screens.json` 的 `interactions` 和 `platform-adaptation.json.interaction_adaptation` 的 `interactions`，逐条检查：
