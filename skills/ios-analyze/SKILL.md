...
其中 `swiftui_view_tree.json` 必须由 SwiftSyntax / SwiftParser 解析 SwiftUI body 得到，是后续 `screens.json.layout_spec` 的首要结构证据。Python 扫描脚本里的正则结果只能作为文件索引和 hints，不能替代 SwiftSyntax View Tree。
...
运行时 UI 树和截图不能替代 SwiftSyntax 或源码核对。UI 树只能说明“运行时渲染出了哪些可访问元素”，截图只能说明“运行时看起来如何”，它们都不能单独决定功能边界、数据来源、模块职责、平台能力或控件 action 的真实业务效果。
...
如果 `swiftui_view_tree.json` 缺失或解析失败，不能假装高精度；必须在 `screens.json.source_evidence`、`visual_notes` 和 `reports/ios界面摘要.md` 中标记“布局结构低置信度”。

如果没有 `runtime-ui/*.json`，必须在 `runtime_ui_tree_evidence` 中标记“运行时 UI 树缺失”；这不阻断功能清单，但后续视觉/可达性验收不能声称已完成运行态结构核对。
...
运行时 UI 树和截图字段可以先为空。它们是辅助证据，不是功能清单成立的前提；但缺失会影响 UI 置信度，必须通过 `layout_confidence`、`runtime_ui_tree_evidence` 和 `screenshot_evidence` 显式反映。
...
如果 SwiftSyntax、运行时 UI 树和截图不一致，保留三种证据：源码结构作为功能事实，UI 树作为运行态结构事实，截图作为视觉事实，并在 `visual_notes` 中标明差异。
...
- SwiftUI 工程缺少 `scan/swiftui_view_tree.json` 且未标记低置信度：失败。
