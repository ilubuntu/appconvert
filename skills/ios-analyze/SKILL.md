...
- `reports/ios工程分析报告.md`：给人和 LLM 建立上下文的辅助报告，不作为事实源。
...
如果需要解释背景、原因、风险或迁移建议，写到 `reports/ios工程分析报告.md`。
...
- 核心 feature 必须有关联 `screens/modules/functions` 和短 key 形式的验收标准 `acceptance`。
...
如果 `swiftui_view_tree.json` 缺失或解析失败，不能假装高精度；必须在 `ios-spec.json.screens.source_evidence`、`visual_notes` 和 `reports/ios工程分析报告.md` 中标记“布局结构低置信度”。
...
资源通过 `usage`、`source_ref` 和 `screens.resource_refs` 关联到页面或功能；不要在每个资源项里重复写 `used_by_features` 或 `screen_id`。
...
python3 skills/ios-analyze/scripts/capture_ios_runtime_ui_tree.py \
  --preflight \
  --project-root {{IOS_PROJECT}} \
  --project {{IOS_PROJECT}}/{{PROJECT_NAME}}.xcodeproj \
...
`ios工程分析报告.md` 是固定产物，用于给人和后续 LLM 快速建立全局上下文。报告可以比 `ios-spec.json` 更丰富，但必须从 `ios-spec.json` 和源码证据派生，不要写成新的事实源。

输出：

- `reports/ios工程分析报告.md`
...
- `reports/ios工程分析报告.md` 有功能、页面、能力或资源但 `ios-spec.json` 没有对应 id：失败。
