...
对后续 skill 暴露的公共契约只有 `specs/*.json`。SwiftSyntax 扫描、运行时 UI 树和截图都属于本 skill 的内部工作证据，必须被折叠进 `specs/screens.json`，不能要求后续 skill 再去读内部目录补信息。
...
后续 agent 只读取 `specs/*.json` 和必要的平台适配产物。`reports/ios分析摘要.md` 只给人快速审阅，不作为事实来源。
...
- 截图能力保留但默认不执行；只有用户明确开启截图或当前任务要求视觉复刻时，才按采集细则执行。
...
其中 `swiftui_view_tree.json` 必须由 SwiftSyntax / SwiftParser 解析 SwiftUI body 得到，是生成 `specs/screens.json.layout_spec` 的首要结构证据。Python 扫描脚本里的正则结果只能作为文件索引和 hints，不能替代 SwiftSyntax View Tree。最终可消费的信息必须进入 `specs/screens.json.source_evidence`、`layout_spec` 和 `component_specs`。
...
运行时 UI 树必须尝试采集，失败不阻塞。失败时字段不能简单留空，必须通过 `runtime_ui_tree_evidence` 记录 `status: "failed"`、`failure_reason` 和已尝试的命令或路径；同时通过 `layout_confidence` 降级反映证据不足。截图默认不执行，`screenshot_evidence` 可为空或标记 `status: "skipped"`。后续阶段不再读取截图目录或 runtime-ui 目录，只读 `specs/screens.json`。
...
每条运行时证据必须写入 screen 的 `runtime_ui_tree_evidence`，这是对后续阶段唯一可见的运行时 UI 树信息：
...
截图是第三层辅助校验项，不是主链路，默认不执行。它用于补充 SwiftSyntax AST 和运行时 UI 树都无法确认的视觉信息，而不是替代源码分析。

只有用户明确要求开启截图，或当前任务要求高保真视觉复刻/视觉比对时，才采集关键页面截图。常见失败原因包括：App 启动失败、没有可用 booted simulator、无法自动进入目标页面、登录/权限弹窗阻塞、loading 未完成、截图命令失败。采集失败不阻塞 `ios-analyze`，但开启截图后必须写入 `screens.json.screenshot_evidence`。
...
截图计划写入 `specs/screens.json` 的 `screenshot_plan` 或 screen 条目中，不固定张数。默认未开启截图时，不生成截图计划。

开启截图后，每条截图证据必须写入 screen 的 `screenshot_evidence`，这是对后续阶段唯一可见的截图信息：
...
Markdown 只用于人审阅，必须从 JSON specs 派生，不要写成唯一事实源。只保留一份摘要文件，避免让后续流程误以为 Markdown 是输入契约。
...
- 截图默认不执行；如果本次开启截图，采集失败必须在 `screenshot_evidence` 记录失败原因，但不能因此跳过功能清单。
