...
必须输出：
- 构建结果
- Harmony 截图产物：必须和 iOS 14 张基准截图一一对应，文件名使用同一个页面编号和 slug
...
- 必须覆盖 output/ios-analyze/ios界面清单.md 中列出的主要页面。
- 必须先生成或采集完整 Harmony 截图，再写“通过”结论。
- Harmony 截图缺失、少于 14 张、或不能和 iOS 截图一一对应时，`output/harmony-visual-verify/界面对齐.md` 的总体结论必须是 `失败`。
